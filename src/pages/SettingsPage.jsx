  ...
  const renderAppearance = () => (
    <>
      ...
      <SettingSection title={tt("Tampilan Pemutar", "Player Look")}>
        ...
        <ToggleRow label={tt("Sampul bergerak (Living Cover)", "Living Cover Motion")} hint={tt("Detail di dalam foto sampul bergoyang halus (efek riak/liquid), bukan foto yang di-zoom, saat lagu diputar", "Details inside the cover art ripple gently in place (a subtle liquid warp, not a zoom), while a song is playing")} checked={settings.livingCover !== false} onChange={set("livingCover")} />
        <ToggleRow label={tt("Sampul animasi (Now Playing & Lirik)", "Animated Artwork (Now Playing & Lyrics)")} hint={tt("Cari video sampul album animasi (kalau tersedia) di layar Now Playing dan tampilan lirik desktop; sampul statis tetap tampil dulu sampai animasinya siap diputar. Pakai data ekstra.", "Looks up an animated album cover video when available on the Now Playing screen and the desktop lyrics view; the static cover always shows first until the animation is ready to play. Uses extra data.")} checked={!!settings.animatedArtwork} onChange={set("animatedArtwork")} />
      </SettingSection>
