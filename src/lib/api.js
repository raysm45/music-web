...
export const LyricsSource = {
  async lrclib({ title, artist, album, duration }) {
    ...
    try {
      ...
      const res = await fetch(`${LRCLIB_BASE}/search?${sq.toString()}`);
      if (res.ok) {
        const list = await res.json();
        const hit = Array.isArray(list) ? list.find((r) => !r.instrumental && (r.syncedLyrics || r.plainLyrics)) : null;
        if (hit) return { syncedLyrics: hit.syncedLyrics || "", plainLyrics: hit.plainLyrics || "" };
      }
    } catch {}
    return null;
  },
  async binilyricsHasMatch({ title, artist, album, duration }) {
    const qs = new URLSearchParams({ title: title || "", artist: artist || "" });
    if (album) qs.set("album", album);
    if (duration) qs.set("duration", String(Math.round(duration)));
    qs.set("source", "lyricsplus,apple,musixmatch,musixmatch-word,spotify");
    try {
      const res = await fetch(`https://lyricsplus.binimum.org/v2/lyrics/get?${qs.toString()}`);
      if (!res.ok) return false;
      const data = await res.json();
      return !!(data && (data.lyrics?.length || data.data?.length || data.lines?.length));
    } catch {
      return false;
    }
  },
